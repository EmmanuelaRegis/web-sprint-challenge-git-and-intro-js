...
function getArtistByIndex(array, number){
  return `the artist at index ${number + 0} is ${array[number].name}`
}
...
function listOfNames(array) {
 ...
 for ( let i = 0; i < array.length; i++) {
  firstNameArray.push(array[i].name)
 }
...
function removeArtist(array, number) {
  const removeable = [...array]
  removeable.splice(array,1)
  return removeable
}
...
function addArtist(array, name, years, genre, nationality, bio) {
  let artistArtist = [...array]
  artistArtist.push({name:name, years:years, genre:genre, nationality:nationality, bio:bio})
  return artistArtist 
  }

console.log(addArtist(artists, "Emma Regis", 2000 - 2022, "Love", "Haitian", 'I just moved to a different city. I am willing to expand my knowledge into technology.'));
...
function artistByCountry(array,nationality){
  ...
  for(let i = 0; i < array.length; i++)
   if(array[i].nationality === nationality){
    fry.push(array[i].name)
   }
   return fry
}
   console.log(artistByCountry(artists, 13));
